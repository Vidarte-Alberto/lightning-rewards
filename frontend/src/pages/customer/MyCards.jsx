    ...
    <div className="flex flex-wrap gap-1.5" aria-label={`${current} of ${required} stamps`}>
      {Array.from({ length: required }, (_, index) => (
        <span
          key={index}
          aria-hidden="true"
          className={`h-3 w-3 rounded-full ${index < current ? 'bg-neutral-900' : 'bg-neutral-200'}`}
        />
      ))}
    </div>
  );
    ...
    <div>
      ...
      {!isLoading && !error && cards.length > 0 && (
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          {cards.map((card) => (
            <article key={card.id} className="rounded-lg border border-neutral-200 bg-white p-5">
              <h2 className="font-semibold text-neutral-900">{card.business.name}</h2>
