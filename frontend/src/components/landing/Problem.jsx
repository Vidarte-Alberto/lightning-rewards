    ...
    <section className="max-w-5xl mx-auto px-6 py-16">
      ...
      <div className="mt-10 grid gap-6 sm:grid-cols-3">
        {PROBLEMS.map((problem) => (
          <div key={problem.title} className="rounded-lg border border-neutral-800 bg-neutral-900/60 p-5">
            <h3 className="font-semibold text-white">{problem.title}</h3>
            <p className="mt-2 text-sm text-neutral-400">{problem.description}</p>
          </div>
        ))}
      </div>
      <p className="mt-10 text-center text-lg font-medium text-amber-400">
